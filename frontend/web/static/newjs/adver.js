...
function AdPrepare(id, title, url, mode, pic, width, height, top, side, autoClose)
{
	...
	document.writeln("<div id='adver_" + id + "' style='z-index:99999; width:" + width + "px; height:" + height + "px; top:" + top + "px; " + (mode == "hangR" ? "right" : "left") + ":"+ side +"px; cursor:pointer; position:absolute; overflow:hidden;'>" + content + "</div>");
...
function AdContent(url, pic, width, height, autoClose)
{
	...
	if (picType == "swf")
	{
		...
		if (url != "")
			if (autoClose)
				content = "<span style='z-index:1;position:absolute;'><a href='" + url + "' target='_blank'><img src='js/blank.gif' width='" + width + "' height='" + height + "' /></a><span style='position:absolute; top:0; right:0;'><img src='js/close.png' /></span></span>" + content;
			else
	...
	else
	{
		...
		if (url != "")
			if (autoClose)
				content = "<a href='" + url + "' target='_blank'>" + content + "</a><span style='position:absolute; top:0; right:0;'><img src='js/close.png' /></span></span>";
			else
